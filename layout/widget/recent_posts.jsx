...
RecentPosts.Cacheable = cacheComponent(RecentPosts, 'widget.recentposts', props => {
    ...
    const posts = site.posts.sort('updated', -1).limit(limit).map(post => ({
        url: url_for(post.link || post.path),
        title: post.title,
        date: date(post.date),
        dateXml: date_xml(post.date),
        thumbnail: has_thumbnail(post) ? get_thumbnail(post) : url_for("img/random/icon (" + Math.floor(Math.random() * 20 + 1) + ").svg"),
